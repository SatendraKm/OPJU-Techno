...
import useFetch from "@/hooks/use-fetch";
import {
  getEventDetailsWithCountsPaginated,
  generateEventCountExcel,
...
export default function EventDetailsPage() {
  ...
  const {
    ...
    error: eventDetailsError,
    fn: eventDetailsFn,
  } = useFetch(getEventDetailsWithCountsPaginated);

  const {
    data: excelData,
    loading: excelLoading,
    fn: excelFn,
  } = useFetch(generateEventCountExcel);

  useEffect(() => {
    const fetchData = async () => {
      await eventDetailsFn(currentPage, 10);
    };
  ...
  useEffect(() => {
    if (eventDetailsData) {
      setEventDetails(eventDetailsData.events);
      setTotalPages(eventDetailsData.totalPages);
      setTotalEvents(eventDetailsData.totalEvents);
    }
  }, [eventDetailsData]);
  ...
  useEffect(() => {
    if (excelData) {
      // Generate Excel from backend data
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>
            Event-Count Details (Page {currentPage} of {totalPages})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="text-sm text-muted-foreground">
              Showing {eventDetails.length} of {totalEvents} events
            </div>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          ...
          >
            {excelLoading ? "Generating..." : "Download as Excel"}
          </Button>
        </CardContent>
      </Card>
    </div>
